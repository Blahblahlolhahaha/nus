...
public class GameTree {
    ...
    int fillNodes(TreeNode node){
        if(node.leaf){
            return node.value;
        }
        Player curr =  node.name.chars().filter(c -> c == 'X').count() == node.name.chars().filter(c -> c == 'O').count() ? Player.ONE : Player.TWO;
        int min = Integer.MAX_VALUE;
        int max = Integer.MIN_VALUE;
        for(TreeNode child : node.children){
            int value = fillNodes(child);
            if(value > max){
                max = value;
            }
            if(value < min){
