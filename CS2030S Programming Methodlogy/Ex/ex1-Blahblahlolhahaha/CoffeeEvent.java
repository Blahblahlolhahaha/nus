...
class CoffeeEvent extends Event {
  ...
  /**
   * Constructor for a coffee shop event.
   *
   ...
   *                    available.
   */
  public CoffeeEvent(int eventType, double time, int customerId, 
  ...
  public String toString() {
    String str = "";
    if (this.eventType == CoffeeEvent.ARRIVAL) {
      str = ": Customer " + this.customerId + " arrives";
    } else if (this.eventType == CoffeeEvent.SERVICE_BEGIN) {
      str = ": Customer " + this.customerId + " service begin (by Counter "
          + this.counterId + ")";
    } else if (this.eventType == CoffeeEvent.SERVICE_END) {
      str = ": Customer " + this.customerId + " service done (by Counter "
          + this.counterId + ")";
    } else if (this.eventType == CoffeeEvent.DEPARTURE) {
      str = ": Customer " + this.customerId + " departed";
    }
  ...
  public Event[] simulate() {
    if (this.eventType == CoffeeEvent.ARRIVAL) {
      ...
      if (counter == -1) {
        ...
        // should depart.
        return new Event[] { 
          new CoffeeEvent(CoffeeEvent.DEPARTURE, this.getTime(), this.customerId)
        };
      } else {
        // Else, the customer should go the the first 
        // available counter and get served.
        return new Event[] { 
          new CoffeeEvent(CoffeeEvent.SERVICE_BEGIN, this.getTime(), this.customerId, 
      ...
      return new Event[] { 
        new CoffeeEvent(CoffeeEvent.SERVICE_END, endTime, this.customerId, 
